<template>
    <article class="page-mock p-2">
        ...
        <div class="d-flex mt-2">
            ...
            <Card  class="ml-2 flex-1" style="background: #eee">
                <TreeDoc class="mt-1" :value="treeData" />
            </Card>
...
export default {
    ...
    methods: {
        syncDb(){
            const {id} = this;
            this.$http.put('/doc', {id,tree:this.treeData});
        },

        async getDoc() {
            const doc = await this.$http.get('/doc', {
                params: { id: this.id },
            });
            this.treeData = doc.tree;
            // console.log(doc);
        },
